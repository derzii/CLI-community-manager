...
> Add the variant to `Screen` enum in `src/app.rs`, wire a key to it
> in `handle_key()` (1–0 are taken; use a letter or F-key), and add a
> tab entry in `render_header()` in `src/ui/mod.rs`. Here's app.rs: [paste]"
...
> "TheBlackroom Rust TUI. Discord client is in src/discord.rs,
> screen renderer in src/ui/screens.rs render_discord(),
> key handler in src/app.rs key_discord(). Here's the relevant code: [paste]"

### "Improve the Database tab"
> "TheBlackroom Rust TUI. Improve the Database tab (screen 8) —
> generic, configurable view over any Notion database. State is the
> db_* fields on App in src/app.rs, DbMode::{Browse, ConfigureColumns,
> SwitchDatabase, EditCell, ViewBody, FilterPresets}. Rendering is
> render_database() in src/ui/screens.rs. Column layout + filter presets
> persist in Profile.database_tab (src/config.rs). Notion calls go through
> `_of`/`_in` NotionClient methods in src/notion.rs. Here's the code: [paste]"

### "Add or modify an automation rule type"
> "TheBlackroom Rust TUI. Automation rules live in src/config.rs
> (AutomationRule, RuleCondition, RuleAction, RuleActionType enum).
> Execution logic is in src/automation.rs run_rule(). The editor overlay
> is AutoRuleForm in src/app.rs, rendered in src/ui/screens.rs
> render_automation(). Here's config.rs and automation.rs: [paste]"

### "Improve analytics"
> "TheBlackroom Rust TUI. Analytics screen is render_analytics() in
> src/ui/screens.rs, key handler is key_analytics() in src/app.rs.
> Data comes from ActivityLogger::analytics_summary() and sparkline()
> in src/analytics.rs. Here's analytics.rs and the relevant app.rs
> sections: [paste]"

### "Improve global search"
> "TheBlackroom Rust TUI. Global search overlay: gs_* fields on App,
> handle_global_search_key() and trigger_global_search() in src/app.rs,
> render_global_search_overlay() in src/ui/screens.rs.
> Four parallel async tasks fan out across Notion (search_global),
> Discord (search_members), FAQ (local), Activity (local).
> Here's app.rs: [paste]"
